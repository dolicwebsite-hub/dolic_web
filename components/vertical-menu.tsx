...
import { Bell, Handshake, Home, PackageSearch, Phone, UserRound } from "lucide-react";

const menuItems = [
  { label: "Trang chủ", href: "/", icon: Home },
  { label: "Sản phẩm", href: "/san-pham", icon: PackageSearch },
  { label: "B2B", href: "/b2b", icon: Handshake },
  { label: "About us", href: "/about-us", icon: UserRound },
  ...
  return (
    <>
      ...
      >
        ...
        <span className="mx-auto my-1 h-px w-7 bg-white/12" />
        <Link
          href="/thu-vien"
          aria-label="Thông báo và thư viện"
          title="Thư viện"
          className={`group relative flex h-11 w-11 items-center justify-center rounded-xl transition ${
            pathname.startsWith("/thu-vien")
              ? "bg-cyan-400 text-slate-950 shadow-[0_16px_32px_-18px_rgba(34,211,238,0.85)]"
              : "text-slate-200 hover:bg-white/10 hover:text-white"
          }`}
        >
      ...
      >
        {menuItems.slice(0, 4).map(({ label, href, icon: Icon }) => {
          const isActive = href === "/" ? pathname === href : pathname.startsWith(href);
        ...
        <Link href="/lien-he" aria-label="Liên hệ" title="Liên hệ" className="flex h-10 w-10 items-center justify-center rounded-xl bg-cyan-400 text-slate-950">
